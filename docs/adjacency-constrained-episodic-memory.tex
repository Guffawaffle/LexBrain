...
\begin{abstract}
Modern AI coding assistants still behave like short-term collaborators: they respond to prompts but cannot reliably resume multi-day engineering work without the user re-explaining what they were doing, why it mattered, and what remains blocked. Separately, most codebases operate under undocumented or tribal architectural rules that are enforced informally (``ask the senior dev if this import is allowed''). This paper introduces a joint system, \textbf{LexBrain} and \textbf{LexMap}, that addresses both problems simultaneously.

LexBrain provides \textit{episodic work memory} as explicit ``Frames'': timestamped, opt-in snapshots of an engineer's working state at a meaningful moment. Each Frame stores (1) a rendered ``memory card'' image, (2) the raw high-signal text behind that image, and (3) structured metadata including branch, blockers, next action, feature flags, and \texttt{module\_scope}. The rendered image exists because vision-capable models can consume high-density panels of text at dramatically lower token cost than equivalent raw text, achieving order-of-magnitude (7--20$\times$) compression in practical long-context workflows, as demonstrated by recent optical/vision context compression research.

LexMap provides \textit{policy-as-code}: a machine-readable file (\texttt{lexmap.policy.json}) that encodes module ownership, allowed and forbidden call edges between modules, and kill patterns to be eliminated from the codebase. Language-specific scanners (PHP, TypeScript, Python, etc.) emit factual observations about code usage, which LexMap then merges and checks against policy.
...
We introduce two cooperating systems:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{LexBrain}: an episodic memory layer that stores \textbf{Frames}. A Frame bundles (a) an opt-in ``memory card'' image summarizing a high-signal moment (logs, failing tests, rationale for leaving a control disabled), (b) the raw text behind that summary, and (c) structured metadata (branch, blockers, \texttt{status\_snapshot.next\_action}, etc.).
    \item \textbf{LexMap}: a policy layer that represents architectural intent as code. It defines modules, their owned paths/namespaces, and which edges are allowed or forbidden. It also tracks feature flags, required permissions, and kill patterns (anti-patterns scheduled for removal).
\end{itemize}

Two ideas unify these systems:
...
The LexBrain memory card is intentionally image-based. Recent work in high-density optical packing of textual context (for example, DeepSeek-OCR-style vision-token compression) shows that large textual state can be delivered to a multimodal model at a fraction of the token cost, while preserving semantic recoverability. This makes multi-day recall economically viable.
...
The system has two cooperating components: LexBrain and LexMap.

\subsection{LexBrain: Episodic Work Memory}
LexBrain captures \textbf{Frames}. A Frame is created intentionally (e.g. via \texttt{/remember}) at a meaningful boundary such as:
\begin{itemize}[leftmargin=1.5em]
...
Frames are stored locally in a developer-owned database (e.g. SQLite such as \texttt{/srv/lex-brain/thoughts.db}). LexBrain exposes Frames to assistants via a Model Context Protocol (MCP) channel over \texttt{stdio} (a spawned local process with environment variables), not via an always-on HTTP service. This local-first posture is intentional: no surveillance, no silent upload of work product.

\subsection{LexMap: Architecture Policy as Code}
LexMap stores architectural boundaries in \texttt{lexmap.policy.json}. Each module describes:
\begin{itemize}[leftmargin=1.5em]
...
LexMap ingests scanner output. Each scanner is language-specific (PHP, TypeScript/JavaScript, Python, etc.) and is \textbf{intentionally dumb}: it walks source files and emits observed facts (declarations, imports, feature-flag checks, permission checks). It does not enforce architecture or infer intent.
...
Concretely:
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{module\_scope} in a LexBrain Frame is an array of module IDs, e.g. \texttt{["ui/user-admin-panel", "services/user-access-api"]}.
    \item Those IDs MUST appear verbatim in \texttt{lexmap.policy.json}.
    \item Violation reports from LexMap must refer to those exact IDs.
...
Starting from \texttt{module\_scope}, LexMap exports a local architectural slice called an \textbf{Atlas Frame}:
\begin{itemize}[leftmargin=1.5em]
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item LexBrain returns the most recent Frame for that ticket, including its memory card image and structured metadata (e.g. \texttt{status\_snapshot.next\_action}).
    \item LexMap uses \texttt{module\_scope} to export an Atlas Frame at fold radius 1.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Intentional Capture, Not Surveillance.} LexBrain does not continuously record keystrokes, shells, or screens. A Frame exists only when the human explicitly triggers it (\texttt{/remember}).
...
\paragraph{Memory Card Rendering Quality.} The memory card images must be legible and consistent, not necessarily pretty. The compression win relies on modern multimodal models' ability to decode structured text from high-resolution panels (similar in spirit to DeepSeek-OCR).
...
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Frames (LexBrain).} Deliberate, timestamped, opt-in episodic memory units that store a compressed, vision-friendly memory card image, raw text context, and structured metadata (branch, blockers, \texttt{status\_snapshot.next\_action}, \texttt{module\_scope}).
    \item \textbf{Policy-as-Code (LexMap).} A version-controlled \texttt{lexmap.policy.json} that encodes architectural ownership, allowed/forbidden edges, required permissions, feature flags, and kill patterns. Enforced in CI.
...
\begin{enumerate}[label={[\arabic*]}, leftmargin=2em]
    ...
    \item Liu, H., et al. \textit{Visual Instruction Tuning}. arXiv:2304.08485, 2023. (Vision-language model foundations)

    \item DeepSeek-AI. \textit{DeepSeek-VL: Towards Real-World Vision-Language Understanding}. arXiv:2403.05525, 2024. (OCR-style multimodal compression)

    \item Anthropic. \textit{Claude 3 Model Family}. Anthropic PBC, 2024. (Multimodal context processing)

    \item OpenAI. \textit{GPT-4 Vision Technical Report}. OpenAI, 2023. (Vision token economics)
    ...
    \item Holzmann, G. J. \textit{The SPIN Model Checker: Primer and Reference Manual}. Addison-Wesley, 2003. (Formal verification and state space exploration)

    \item Flanagan, C., et al. \textit{Extended Static Checking for Java}. ACM SIGPLAN Notices, 2002. (Architecture conformance foundations)

    \item Nature Portfolio. \textit{Authorship: AI tools and services}. Springer Nature, 2023. \url{https://www.nature.com/nature-portfolio/editorial-policies/authorship} (Publication policy on AI assistance disclosure)

    \item Science Magazine. \textit{Generative AI and scientific publishing}. AAAS, 2023. (Guidelines for AI tool acknowledgment)
\end{enumerate}
